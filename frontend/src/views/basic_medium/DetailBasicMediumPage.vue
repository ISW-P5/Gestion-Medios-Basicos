    ...
    <CCard v-else-if="this.$route.name === (route + '.detail')">
        ...
        <CCardBody>
            <CDataTable :items="detail" :fields="fields" :noItemsView="{
                ...
                <template #p="{item}">
                    <td v-if="item.v && item.v !== 'None' && item.v !== ''" class="text-right col-md-4">
                        <b>{{item.p}}</b>
                    </td>
                </template>
                <template #v="{item}">
                    <td v-if="item.v && item.v !== 'None' && item.v !== ''" class="col-md-8">{{item.v}}</td>
                </template>
            </CDataTable>
        </CCardBody>
    </CCard>
...
export default {
    ...
    methods: {
        ...
        get_queryset() {
            this.$services.get_BasicMedium(this, this.$route.params.id);
        },
        metadata_queryset() {
            this.$services.metadata_BasicMedium(this);
        },
    },
    created() {
